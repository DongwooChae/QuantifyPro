async function loadCategoryAndFsdata(filter = '', group = '') {
    try {
        ...
        // 2) URL 생성
        let url = 'http://115.68.219.19:8080/api/categoryandfsdata';
        if (params.length) {
            url += '?' + params.join('&');
        }
        ...
        const displayColumns = [
            'kap_group', 'company', 'sector', 'industry', 'main_product', 'core_sales' , 'PER','PSR','PBR'
        ];
...
async function loadGroupOptions() {
    const res = await fetch('http://115.68.219.19:8080/api/groupnames');
